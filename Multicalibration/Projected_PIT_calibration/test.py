...
def ensemble_PIT(samples, y):
    d = min(y.shape[1], 3)
    pca = PCA(n_components=d) #keeping all components, 4 in this case
    pca.fit(samples.reshape(-1,d))
...
def pce(model, dataset):
    ...
    total_pits = torch.cat(total_pits, dim=1) # (3, N)
...
def objective(trial):
    lambda_reg = trial.suggest_float('lambda_reg', 1e-4, 10, log=True)
