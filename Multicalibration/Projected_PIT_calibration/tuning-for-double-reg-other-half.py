...
# datasets = [
#             # ['camehl', 'households'], ['cevid', 'air'], ['cevid', 'births1'],
...
# results_path = "tuning-results-binary.csv"
...
results_path = "tuning-results-double-reg-otherhalf2.csv"
# Load existing results if the file exists
if os.path.exists(results_path):
    df = pd.read_csv(results_path)
else:
...
for data_group, data_name in datasets:
    ...
    for prerank in preranks:
            for l in lambdas:
                ...
                model = MixtureLightningModule(p, q, lambda_reg=l, reg_type='pce-kde', prerank=prerank, double_reg=True)
                trainer = get_lightning_trainer(rc)
                ...
                try:
                    ...
                    for x, y, idx in val_loader:
                        ...
                        pce_val = prerank_pce.mean() + marg_pce.mean()

                        energy_val = multivariate_energy_score(dist, y, n_samples=best_model.hparams.es_num_samples).mean()
                        
                        all_pce.append(pce_val.item())
                        all_energy.append(energy_val.item())
