...
from scoring_rules import BrierScoreLoss, RPS, TemperatureScaler
...
class SimpleMLP(BaseEstimator):
    def __init__(self, is_classification: bool, device: str = 'cpu', loss_type = 'cross_entropy'):
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        ...
        for epoch in range(n_epochs):
            ...
            with torch.no_grad():
                ...
                scaled_logits = calib.predict_proba_torch(CategoricalLogits(logits)).get_logits()
                ...
                wandb.log({"train_loss": avg_train_loss, "valid_loss": valid_loss, 
                           "refinement_error" : refinement_loss, "calibration_error": calib_loss, "epoch": epoch + 1})
...
class Standalone_RealMLP_TD_S_Classifier(BaseEstimator, ClassifierMixin):
    def __init__(self, device: str = 'cpu', loss_type = 'cross_entropy'):
        self.device = device
        self.loss_type = loss_type

    def fit(self, X, y, X_val=None, y_val=None):
        self.prep_ = get_realmlp_td_s_pipeline()
        self.model_ = SimpleMLP(is_classification=True, device=self.device, loss_type=self.loss_type)
        X = self.prep_.fit_transform(X)
        if X_val is not None:
            X_val = self.prep_.transform(X_val)
        print("Right before fitting")
